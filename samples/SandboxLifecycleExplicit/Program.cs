...
using SandboxLifecycleExplicit;

// =============================================================================
// The sandbox lifecycle, STEP BY STEP — no Fake* types, and no facade either.
//
// SandboxAgentHost.RunAsync() does all of this in one call (see SandboxRunnerHostMinimal, which is what
// you should copy). This sample keeps the steps written out, for when you need to own one of them:
// your own admission control, your own online-wait or provisioning telemetry, a warm pool, a reaper.
// Every call below is public API — the facade is a convenience over exactly these, not a wall.
//
...
app.Logger.LogInformation("SandboxLifecycleExplicit is up. Provision a sandbox + run one turn:");
app.Logger.LogInformation("  curl -X POST 'http://localhost:5086/demo/sandbox-run?repo=<git-url>&prompt=hi'");
